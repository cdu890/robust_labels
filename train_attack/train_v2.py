...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--label",
        type=str,
        help="Label in [speech, uniform, shuffle, composite, random, uniform, lowdim, bert, glove, normal]",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        default="./labels/label_files/new",
    )
    parser.add_argument(
        "--data_dir",
        type=str,
    ...
    train_log_folder = base_dir / "log"
    train_log_path = os.path.join(train_log_folder, "{}_{}_train_log.csv".format(dataset, label))
    if not os.path.exists(train_log_path):
        with open(train_log_path, "w") as train_log_file:
            train_log_writer = csv.writer(train_log_file, delimiter=",")
            header = ["Epoch", "Train Loss", "Valid Loss", "Valid Acc"]
    ...
    for epoch in range(epoch_start, epoch_stop):
        ...
        with open(train_log_path, "a") as train_log_file:
            train_log_writer = csv.writer(train_log_file, delimiter=",")
            row = [epoch+1, new_train_loss, new_valid_loss, new_valid_acc]
            train_log_writer.writerow(row)
